...
def get_dataloader(params, mode="train"):
    ...
    if params.dataset == "gaze360":
        dataset = Gaze360(
            ...
            mode=mode,
            poison_rate=params.poison_rate if mode == "train" else 0.0,
            seed=getattr(params, "seed", 42),
            poison_target=tuple(params.poison_target),
            trigger_test=(mode != "train" and getattr(params, "trigger_test", False))
        )
...
def draw_gaze(frame, bbox, pitch, yaw, thickness=2, color=(0, 0, 255)):
    ...
    cv2.arrowedLine(frame, point1, point2,
...
def draw_bbox_gaze(frame: np.ndarray, bbox, pitch, yaw, color=(0, 0, 255), thickness=2):
    """Draw both bounding box and gaze arrow, with configurable arrow color."""
    draw_bbox(frame, bbox, color=(0, 255, 0), thickness=1)  # bbox in green
    draw_gaze(frame, bbox, pitch, yaw, thickness=thickness, color=color)